...
// TODO: Add another fork for consolidation block.
 ...
 * @param {number} opts.withdrawableEpoch - FIXME: Does it make sense to keep as an input?
 */
function main(opts) {
  assert(opts);
  ...
  // XXX: What if validator did not exist at this point, hence the balance is 0?
  ...
  for (let s = 0; s < state.slot; s += SLOTS_PER_HISTORICAL_ROOT) {
    ...
    if (shouldPatchSummary) {
      ...
      state.tree.setNode(nav.gindex, BlockRoots.toView(blockRoots).node); // FIXME: Check that it doesn't change state.root.
    } else {
      state.historicalSummaries.push(summary);
    }
  }
